...
// Handle Nav Tabs (Home, Sign Up, Log In)
tabs.forEach((tab, index) => {
    tab.addEventListener("click", () => {
        ...
        // Show the active section based on tab index
        if (index === 0) {
...
// Link "Get Started" button to the Sign Up tab
...
async function renderTasks(tasks) {
    ...
    taskContainer.innerHTML = tasks.map((task, i) => {
        return `
        <div class="task group relative bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-white/15 hover:border-violet-400/30 hover:shadow-violet-500/10 transition-all duration-300 ease-out shadow-lg shadow-black/10 hover:-translate-y-1 overflow-hidden snap-start">
            <div class="flex flex-col gap-1">
                <div class="flex flex-wrap items-center gap-2">
                    <span class="task-id bg-violet-500/20 text-violet-300 text-xs px-2 py-1 rounded-lg font-semibold">
                        ${task.id}
                    </span>
                ...
                <div class="text-sm text-gray-300 flex flex-wrap gap-3">
                    <span class="task-date">${task.AddedDate}</span>
                    <span class="task-time">${task.AddedTime}</span>
                </div>
            </div>
            <button data-id="${task.id}" class="RemoveTaskBtn w-full sm:w-auto bg-red-500/20 hover:bg-red-500 text-red-300 hover:text-white px-4 py-2 rounded-xl transition-all duration-300 font-medium active:scale-95 hover:shadow-lg hover:shadow-red-500/30">
                Remove
            </button>
        </div>`;
    }).join("");
...
logout.addEventListener("click", () => {
    ...
    // Auto-return to Home view on logout
    tabs[0].click(); 
});
